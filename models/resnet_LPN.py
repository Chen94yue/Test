...
class resnet_LPN(nn.Module):
    def __init__(self, num_classes):
        super(resnet_LPN,self).__init__()
        ...
        self.Convmask = nn.Conv2d(2048, 49, 1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        #x = x5*mask
        x = x5
        location = self.Convmask(x)
        location = self.avgpool2(location)
        # mask = F.tanh(mask)
        location = torch.argmax(location, dim=1).float()
        location = location.view(location.size(0),-1)
        location = (location - 24)/49
